...
// SCANF_WINDOW
    while (inf >> dna_s)
    {
        ...
        while (index_s < dna_s.size()) 
        {
            ...
            if (dna_ref_b >= PointerListLen) n = to_bit(dna_w.substr(dna_w.size() - PointerListLen, PointerListLen));
            if (dna_ref_b > WindowListLen / 2 && ((dna_ref_b - 1 - WindowListLen / 2) % WindowListLen + 1 >= PointerListLen))
            {
                if ((dna_ref_b - 1) % WindowListLen + 1>= PointerListLen)
                {
                    if ((dna_ref_b / WindowListLen) * 2 < 2 * ((dna_ref_b - WindowListLen / 2 ) / WindowListLen) + 1)
                    {
                        ...
                        db.link_string(n, 2 * ((dna_ref_b - 1- WindowListLen / 2 ) / WindowListLen) + 1);
                    }
                    else
                    {
                        db.link_string(n, 2 * ((dna_ref_b - 1 - WindowListLen / 2 ) / WindowListLen) + 1);
                ...
                if (dna_ref_b >= WindowListLen)
                    if (dna_ref_b % (WindowListLen / 2) == 0)
                        if (dna_ref_b % (WindowListLen) == 0)
                            outf << dna_w.substr(n - WindowListLen, WindowListLen) << " " <<  (dna_ref_b / WindowListLen) * 2 - 2<< "| \n";
                        else
                            outf << dna_w.substr(n - WindowListLen, WindowListLen) << " " <<  2 * ((dna_ref_b - WindowListLen / 2 ) / WindowListLen) - 1<< "| \n";
            #endif
        }
